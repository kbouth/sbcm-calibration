...
def Aselect_function(eclick, erelease):
    global Axmin
    global Axmax
    extent = A_selector.extents
    Axmin=round(extent[0],3)
    Axmax=round(extent[1],3)
    
def Bselect_function(eclick, erelease):
    global Bxmin
    global Bxmax
    extent = B_selector.extents
    Bxmin=round(extent[0],3)
    Bxmax=round(extent[1],3)
...
while True:
    if(clear==0):
        if(pause==0):
            ...
            ax[0].set_ylabel("EPS PLC DCCT Current (mA)")
            ...
            bx[0].set_ylabel("EPS PLC DCCT Current (mA)")
            bx[0].set_title("SBCM-B "+pattern+"% Fill Beam Scan")
        else:
            if A_selector is None:
                A_selector = RectangleSelector(ax[0],Aselect_function,
                    button=[1],interactive=True)
            if(Axmax>Axmin):
                VFAroi=[]
                IBMAroi=[]
                for i in range(0,len(VFA)):
                    if(VFA[i]>=Axmin and VFA[i]<=Axmax):
                        VFAroi.append(VFA[i])
                ...
                ax[1].set_ylabel("ROI: EPS PLC DCCT Current (mA)")
                if(len(VFAroi)>2):
            ...
            if B_selector is None:
                B_selector = RectangleSelector(bx[0],Bselect_function,
                    button=[1],interactive=True)
            if(Bxmax>Bxmin):
                VFBroi=[]
                IBMBroi=[]
                for i in range(0,len(VFB)):
                    if(VFB[i]>=Bxmin and VFB[i]<=Bxmax):
                        VFBroi.append(VFB[i])
                ...
                bx[1].set_ylabel("ROI: EPS PLC DCCT Current (mA)")
                if(len(VFBroi)>2):
                    m,b = np.polyfit(VFBroi,IBMBroi,1)
                    correlation_matrix = np.corrcoef(VFBroi,IBMBroi)
                    p = correlation_matrix[0, 1]
                    bx[1].text(0.05, 0.93, "Slope:"+str(round(m,5))+"mA/mVfa",transform=bx[1].transAxes, fontsize=12,verticalalignment='top', bbox=props)
